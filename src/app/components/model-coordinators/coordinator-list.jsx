...
export function CoordinatorList({
  coordinators,
  listClassName = "grid sm:grid-cols-2  lg:grid-cols-3 gap-8 ",
}) {
    ...
    <div className="min-h-screen flex flex-col justify-between">
      <div>
        <h1 className="px-3 py-3">
          Showing 12 results of {coordinators.length} profiles
        </h1>
...
export function CoordinatorCard({ coordinator }) {
  const ProfileIcon = coordinator.userType === "freelancer" ? UserIcon : BusinessIcon
  return (
   
      <div className=" border  rounded-lg shadow hover:shadow-md transition-shadow duration-300">
        <div className="relative w-full max-w-sm bg-white p-3 flex flex-col items-center">
          ...
          <div className="flex items-center gap-1 text-gray-600 mb-2">
            ...
            <p className="text-gray-600">{coordinator.address.city}</p>
          </div>

          <h3 className="custom-h5 font-semibold text-fontColor-secondary mb-1">{coordinator.name}</h3>

          <p className="text-gray-500 mb-6">{coordinator.role}</p>

    

          <Link
           href={`/model-coordinators/${coordinator.id}`}
